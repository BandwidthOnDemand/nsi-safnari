...
import net.nordu.namespaces._2013._12.gnsbod.{
  ConnectionTraceType,
  ConnectionType,
  ObjectFactory as ConnectionTraceTypeOF
}
...
class ConnectionEntitySpec extends helpers.ConnectionEntitySpecification:
  "A connection" >> {
      ...
      "send a path computation request when reserve is received" in new fixture:
        ...
        when(
          ura.request(
            ...
            Nil,
            new ConnectionTraceTypeOF().createConnectionTrace(
              new ConnectionTraceType().withConnection(connectionTrace)
            ) :: Nil
          )
        )
      ...
      "send reserve request including the connection trace" in new fixture:
        ...
        messages must contain(like[Message] {
          case ToProvider(NsiProviderMessage(headers, _), provider) if provider == A.provider =>
            ...
            headers.connectionTrace must contain(
              equalTo(new ConnectionType().withIndex(3).withValue(s"$AggregatorNsa:$ConnectionId"))
            )
            headers.connectionTrace must contain(
      ...
      "fail the connection when path computation fails" in new fixture:
        ...
        messages must contain(like[Message] {
          case ToRequester(NsiRequesterMessage(headers, ReserveFailed(failed))) =>
            ...
            failed.getServiceException().getErrorId() must beEqualTo(
              NsiError.NoServicePlanePathFound.id
            )
        })
    ...
    "in reserve checking state" should {
      ...
      "notify timeout to requester when child times out" in new fixture:
        ...
        `given`(
          ...
          agg.request(
            CorrelationId(0, 4),
            InitialReserve(
              InitialReserveType.tap(_.getCriteria.withPointToPointService(A.serviceType.service))
            )
          ),
          pce.confirm(CorrelationId(0, 3), A)
        )
            ...
            ReserveConfirmed(
              ...
              ConfirmCriteria.withPointToPointService(
                A.serviceType.service
                  ...
                  .withDestSTP("networkId:A:B?vlan=12")
              )
            )
          )
        )
        ...
        when(
          B.provider.response(
            ...
            ReserveConfirmed(
              ...
              ConfirmCriteria.withPointToPointService(
                A.serviceType.service
                  .withSourceSTP("networkId:B:A?vlan=12")
      ...
      "fail the reservation with a single path segment" in new fixture:
        ...
        messages must contain(like[Message] {
          case ToRequester(NsiRequesterMessage(headers, ReserveFailed(failed))) =>
            ...
            failed.getServiceException().getChildException().get(0).getErrorId() must beEqualTo(
              NsiError.CAPACITY_UNAVAILABLE.id
            )
        })
        reservationState must beEqualTo(ReservationStateEnumType.RESERVE_FAILED)
          ...
          upa.response(
            ...
            ReserveFailed(
              new GenericFailedType()
                ...
                .withServiceException(
                  NsiError.CapacityUnavailable(1000).toServiceException(B.provider.nsa)
                )
            )
          )
        )
      ...
      "pass child reserve timeout to requester for each segment" in new fixture:
          ...
          upa.notification(
            ...
            ReserveTimeout(
              new ReserveTimeoutRequestType()
                ...
                .withNotificationId(32L)
                .withTimeStamp(
                  DatatypeFactory.newInstance().newXMLGregorianCalendar("2002-10-09T11:15:00Z")
                )
        ...
        childConnectionData(
          "ConnectionIdB"
        ).reservationState aka "child B reservation state" must beEqualTo(
          ReservationStateEnumType.RESERVE_CHECKING
        )
        reservationState must beEqualTo(ReservationStateEnumType.RESERVE_CHECKING)
    ...
    "in reserve held state" should {
      ...
      "be in reserve timeout state when provider times out" in new fixture:
        ...
        childConnectionData(
          ...
          ReservationStateEnumType.RESERVE_TIMEOUT
        )
    ...
    "in reserve committing state" should {
      "be in reserved state when reserve commit confirmed is received" in new fixture:
        ...
        messages must contain(
          ToRequester(
            NsiRequesterMessage(
              nsiRequesterHeaders(CommitCorrelationId),
              ReserveCommitConfirmed(ConnectionId)
            )
          )
        )
    ...
    "in reserve failed state" should {
          ...
          upa.response(
            ...
            ReserveFailed(
              new GenericFailedType()
                .withConnectionId("ConnectionIdA")
                .withServiceException(
                  NsiError.CapacityUnavailable(2000).toServiceException(A.provider.nsa)
                )
            )
          )
        )
      ...
      "be in reserve start state when reserve abort is received with a single child connection without connection id" in new fixture:
        `given`(
          ...
          upa.acknowledge(
            CorrelationId(0, 4),
            ServiceException(NsiError.GenericMessagePayloadError.toServiceException(A.provider.nsa))
          )
        )
    ...
    "in reserve aborting state" should {
      "be in reserve start state when reserve abort confirm is received" in new fixture:
        ...
        reservationState must beEqualTo(ReservationStateEnumType.RESERVE_START)
        childConnectionData("ConnectionIdA").reservationState must beEqualTo(
      ...
      "be in reserve start state when reserve abort confirm is received from two two children" in new ReserveHeldConnectionWithTwoSegments:
        ...
        childConnectionData("ConnectionIdB").reservationState must beEqualTo(
          ReservationStateEnumType.RESERVE_ABORTING
        )
    ...
    "that has been committed" should {
      ...
      "be in terminating state when terminate is received" in new ReservedConnection:
        ...
        childConnectionData("ConnectionIdA").lifecycleState must beEqualTo(
        ...
        messages must contain(
          ToProvider(
            NsiProviderMessage(
              toProviderHeaders(A.provider, CorrelationId(0, 8)),
              Terminate("ConnectionIdA")
            ),
            A.provider
          )
        )
      ...
      "be in terminating state when terminate is received (multi segment)" in new ReservedConnectionWithTwoSegments:
        ...
        childConnectionData("ConnectionIdB").lifecycleState must beEqualTo(
          LifecycleStateEnumType.TERMINATING
        )
        ...
        messages must contain(
          exactly[Message](
            ...
            ToProvider(
              NsiProviderMessage(
                toProviderHeaders(B.provider, CorrelationId(0, 12)),
                Terminate("ConnectionIdB")
              ),
              B.provider
            )
          )
    ...
    "data plane state machine (DSM)" should {
          ...
          upa.notification(
            ...
            DataPlaneStateChange(
              new DataPlaneStateChangeRequestType()
                ...
                .withDataPlaneStatus(dataPlaneStatusType(true, consistent = true))
                .withTimeStamp(
                  DatatypeFactory.newInstance().newXMLGregorianCalendar("2002-10-09T11:10:00Z")
                )
            )
          )
        )
      ...
      "be in terminating state when terminate is received" in new ReservedConnection
        ...
        messages must contain(
          ToProvider(
            NsiProviderMessage(
              toProviderHeaders(A.provider, CorrelationId(0, 12)),
              Terminate("ConnectionIdA")
            ),
            A.provider
          )
    ...
    "in terminating state" should {
      "send a terminate confirmed to requester when terminate confirmed is received" in new ReservedConnection:
        ...
        messages must contain(
          ToRequester(
            NsiRequesterMessage(
              nsiRequesterHeaders(TerminateCorrelationId),
              TerminateConfirmed(ConnectionId)
            )
          )
        )
    ...
    "in created state" should {
      "become failed on ForcedEnd error event" in new ReservedConnection:
        ...
        when(
          upa.notification(
            ProviderErrorEventCorrelationId,
            ErrorEvent(
              new ErrorEventType()
                .withConnectionId("ConnectionIdA")
        ...
        connection.lsm.childConnectionState("ConnectionIdA") must beEqualTo(
    ...
    "in released state" should {
      ...
      "send a provision confirmed to requester" in new ReservedConnection with Released:
        ...
        messages must contain(
          ToRequester(
            NsiRequesterMessage(
              nsiRequesterHeaders(ProvisionCorrelationId),
              ProvisionConfirmed(ConnectionId)
            )
          )
        )
    ...
    "in provisioned state" should {
      "become releasing on release request" in new ReservedConnection with Provisioned:
        ...
        messages must contain(
          ToProvider(
            NsiProviderMessage(
              toProviderHeaders(A.provider, CorrelationId(0, 10)),
              Release("ConnectionIdA")
            ),
            A.provider
          )
        )
      ...
      "send release confirmed to requester" in new ReservedConnection with Provisioned:
        ...
        messages must contain(
          ToRequester(
            NsiRequesterMessage(
              nsiRequesterHeaders(ReleaseCorrelationId),
              ReleaseConfirmed(ConnectionId)
            )
          )
        )
  ...
  "Connection entity" should {
    ...
    "send query recursive to all child providers" in new ReservedConnectionWithTwoSegments:
      ...
      messages must contain(like[Message] {
        case ToProvider(
              NsiProviderMessage(_, QueryRecursive(Some(Left("ConnectionIdA" :: Nil)), None)),
              provider
            ) =>
          provider must_== A.provider
        case ToProvider(
              NsiProviderMessage(_, QueryRecursive(Some(Left("ConnectionIdB" :: Nil)), None)),
              provider
            ) =>
