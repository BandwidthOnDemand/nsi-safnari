  ...
  "A connection" >> {
    "that has been committed" should {
                  ...
                  new ReserveType()
                    ...
                    .withCriteria(
                      new ReservationRequestCriteriaType()
                        ...
                        .withSchedule(
                          new ScheduleType().withStartTime(Nillable.absent[XMLGregorianCalendar])
                        )
                    )
                )
              ),
      ...
      "fail modify when requested version is less than committed version" in new ReservedConnection {
        when(
          ura.request(
            ModifyCorrelationId,
            ModifyReserve(ModifyReserveType.tap(_.getCriteria.setVersion(3)))
          )
        )
    ...
    "in modifying state" should {
      "become reserve held when modify is confirmed" in new ReservedConnection with Modified {
        when(
          upa.response(
            ...
            ReserveConfirmed(
              "ConnectionIdA",
              ModifyReserveType.getCriteria
                .toModifiedConfirmCriteria(connection.rsm.committedCriteria.get)
                .get
            )
          )
        )
      ...
      "become committed when modify commit is confirmed" in new ReservedConnection with Modified {
        ...
        when(upa.response(CorrelationId(0, 9), ReserveCommitConfirmed("ConnectionIdA")))
          ...
          upa.acknowledge(
            ...
            ServiceException(
              NsiError.NotImplemented
                .toServiceException(A.provider.nsa)
                .withConnectionId("ConnectionIdA")
            )
          )
        )
